...
fn place_antinode(char_array: &mut Vec<Vec<char>>, antenna1: &Position, antenna2: &Position) {
    let mut antinode1: Position = Position { x: 0, y: 0 };

    antinode1.x = antenna1.x + (antenna1.x - antenna2.x);
    antinode1.y = antenna1.y + (antenna1.y - antenna2.y);

    if check_array_bounds(antinode1.x, antinode1.y, &char_array) {
...
mod tests {
    ...
    fn test_website_example_1() {
        ...
        assert_eq!(14, sum_antinodes(&char_array));
    }
}
