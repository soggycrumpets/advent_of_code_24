...
fn get_sequence_from_keypad(input_sequence: &str, keypad_type: KeypadType, mode: bool) -> String {
    ...
    for button in input_sequence.chars() {
        let (directions, new_position) = navigate_keypad(button, position, keypad_type, mode);
...
fn main() {
    ...
    let mut final_sequences_1: Vec<String> = Vec::new();
    for sequence_1 in &numpad_sequences {
        let sequence_2 = get_sequence_from_keypad(&sequence_1, KeypadType::Numerical, false);
        let sequence_3 = get_sequence_from_keypad(&sequence_2, KeypadType::Directional, false);
        let sequence_4 = get_sequence_from_keypad(&sequence_3, KeypadType::Directional, false);
        println!("{}: {}", sequence_4.len(), sequence_4);
        final_sequences_1.push(sequence_4);
    }

    let mut final_sequences_2: Vec<String> = Vec::new();
    for sequence_1 in &numpad_sequences {
        let sequence_2 = get_sequence_from_keypad(&sequence_1, KeypadType::Numerical, true);
        let sequence_3 = get_sequence_from_keypad(&sequence_2, KeypadType::Directional, true);
        let sequence_4 = get_sequence_from_keypad(&sequence_3, KeypadType::Directional, true);
        println!("{}: {}", sequence_4.len(), sequence_4);
        final_sequences_2.push(sequence_4);
    }

    let mut final_sequences_2_iter = final_sequences_2.into_iter();
    for sequence_1 in &mut final_sequences_1 {
        let sequence_2 = final_sequences_2_iter.next().unwrap();
        if sequence_1.len() > sequence_2.len() {
            *sequence_1 = sequence_2;
        }
    }

    // Get the complexity
    let complexity = compute_complexity(numpad_sequences, final_sequences_1);
