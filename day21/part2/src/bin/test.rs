...
fn find_shortest_length(sequences: &Vec<String>) -> usize {
...
fn filter_unoptimal_sequences(sequences: Vec<String>) -> Vec<String> {
    let shortest_len = find_shortest_length(&sequences);
    println!("Shortest sequence length: {}", shortest_len);
    let mut filtered_sequences: Vec<String> = Vec::new();
    for sequence in sequences {
        if sequence.len() == shortest_len {
...
fn get_shortest_sequence_len(input: &Vec<char>) -> usize {
    ...
    let mut sequences1 = get_shortest_sequences(&input, Keypad::Numerical);
    sequences1 = filter_unoptimal_sequences(sequences1);
    for sequence in &sequences1 {
        println!("{}", sequence);
    }

    let mut sequences2 = get_shortest_sequences(
        &sequences1[5].chars().collect::<Vec<char>>(),
        Keypad::Directional,
    );
    ...
    // }
    sequences2 = filter_unoptimal_sequences(sequences2);
    ...
    sequences3 = filter_unoptimal_sequences(sequences3);

    // Find the shortest length of all the sequences for the third set
    let min_sequence_len = find_shortest_length(&sequences3);
    min_sequence_len
}

fn main() {
    let all_numpad_sequences = load_button_sequences(_EXAMPLE2);
    let mut numpad_sequences = Vec::new();
    numpad_sequences.push(all_numpad_sequences[4].clone());
