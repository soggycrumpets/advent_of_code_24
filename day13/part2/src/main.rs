...
// One of the buttons is marked "x-dominant" and the other is marked "y-dominant"
// The x-dominant button is pressed/unpressed until the current x-position matches (or gets as close as it can get to) the prize's x-position
// Then, the y-dominant button is pressed/unpressed until the current y-position matches the prize's y-position
...
// This function is called twice, once with button A being "x-dominant" and the other with B being "x-dominant".
...
fn search_for_equilibrium(
    ...
    x_dominant_button: &Position,
    y_dominant_button: &Position,
) -> Option<(i64, i64)> {
    ...
    for _i in 0..100 {
        ...
        let x_dominant_button_presses =
            (amount_to_move_x as f64 / x_dominant_button.x as f64).floor() as i64; // This floor() is important. The function will fail without it.
        total_x_dominant_button_presses += x_dominant_button_presses;

        match i64::checked_mul(x_dominant_button_presses, x_dominant_button.x) {
            Some(mul) => current_position.x += mul,
            None => break,
        }
        match i64::checked_mul(x_dominant_button_presses, x_dominant_button.y) {
        ...
        let y_dominant_button_presses =
            (amount_to_move_y as f64 / y_dominant_button.y as f64).ceil() as i64; // Same with this ceil()
        total_y_dominant_button_presses += y_dominant_button_presses;

        match i64::checked_mul(y_dominant_button_presses, y_dominant_button.x) {
            Some(mul) => current_position.x += mul,
            None => break,
        }
        match i64::checked_mul(y_dominant_button_presses, y_dominant_button.y) {
            Some(mul) => current_position.y += mul,
            None => break,
        }
    ...
    if current_position == claw_machine.prize {
        return Some((
            total_x_dominant_button_presses,
            total_y_dominant_button_presses,
        ));
    } else {
        return None;
    }
...
fn compute_minimum_cost(claw_machine: ClawMachine) -> i64 {
    ...
    // A as x-dominent converges on a solution
    ...
    // B as x-dominant converges on a solution
